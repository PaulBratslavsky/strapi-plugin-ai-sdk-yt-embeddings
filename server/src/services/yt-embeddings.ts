...
const ytEmbeddings = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async embedTranscript(transcript: TranscriptInput): Promise<{ videoId: string; chunkCount: number; skipped: boolean }> {
    ...
    try {
      ...
      const config = strapi.config.get('plugin::ai-sdk-yt-embeddings') as any;
      let topics: string[] = [];
      let summary = '';
  ...
  async recomputeAll() {
    ...
    const transcripts = await strapi.documents('plugin::ai-sdk-yt-transcripts.transcript' as any)
