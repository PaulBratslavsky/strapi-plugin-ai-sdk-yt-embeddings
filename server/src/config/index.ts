...
export default {
  ...
  validator(config: PluginConfigSchema) {
    if (!config.openAIApiKey) {
      console.warn(
        "ai-sdk-yt-embeddings: openAIApiKey is not configured. Plugin features will be disabled."
      );
    }
    if (!config.neonConnectionString) {
      console.warn(
        "ai-sdk-yt-embeddings: neonConnectionString is not configured. Plugin features will be disabled."
      );
    }
    if (config.embeddingModel && !EMBEDDING_MODELS[config.embeddingModel]) {
      console.warn(
        `ai-sdk-yt-embeddings: Invalid embeddingModel "${config.embeddingModel}". ` +
    ...
    if (config.chunkSize && (config.chunkSize < 100 || config.chunkSize > 8000)) {
      console.warn(
        `ai-sdk-yt-embeddings: chunkSize ${config.chunkSize} is outside recommended range (100-8000). ` +
